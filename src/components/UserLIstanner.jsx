...
const UserLIstanner = () => {
  ...
  const {searchName , setSearchName}= useGlobalHook()
  return (
...
<div className='flex items-center justify-between px-8  py-7 h-[6.125rem] border-b border-b-[#e5e5e5] bg-white example'>
    ...
    <SearchIcon/>
       <input type="text" value={searchName} onChange={(e)=>setSearchName (e.target.value)} className='h-full outline-none rounded-r-full px-2 z-10 relative left-[2px]' />
      </div>
</div>
  )
}
